...
var client = function (config, httpOptions) {
	...
	if (config.prefix !== 's3') {
		...
		config.request = function (action, query, callback) {
			...
			tools.debug('request body: ' + clientBody);
			
			// The Expect: 100-continue header MUST be disabled for STS, SNS, DynamoDB.
			// The server returns 500 as the AWS engineers were to lazy to properly use HTTP.
			if ( ! config.noContinue) {
				headers.expect = '100-continue';
			}
			
			options = lodash.clone(httpOptions);
			options.method = 'POST';
			options.headers = tools.merge(headers, clientHeaders);

			if (config.host.indexOf('eu-central') !== -1) {
				options.host = config.host;
				options.body = clientBody;
				aws4.sign(options, config);
			}
	...
	if (config.prefix === 's3') {
		...
		config.signUrl = function (protocol, method, path, expires, headers) {
			...
			if (protocol === 'http') {
				return 'http://' + config.host + path + separator + signature;
			}
			
			path = '/' + config.getBucket() + path;
			
			return 'https://' + config.prefix + cfg.suffix + path + separator + signature;
		};
		...
		config.putFiles = function (files, acl, headers, callback) {
			...
			var collectResult = function (error, result, index) {
				errors[index] = error;
			...
			files.forEach(function (element, index) {
				config.putFile(element.path, element.file, acl, headers, function (error, result) {
					fileCounter--;
					collectResult(error, result, index);
				});
			});
		};
		 ...
		 * @param {Function} callback
		 */
		config.delMultiObjects = function (objects, quiet, callback) {
			...
			config.post('?delete', headers, xml, callback);
		};
